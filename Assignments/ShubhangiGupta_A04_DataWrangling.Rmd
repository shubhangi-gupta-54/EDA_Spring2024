...
```{r}
EPAair_O3_NC2018_processed <- EPAair_O3_NC2018_raw[,c("Date", "DAILY_AQI_VALUE",
"Site.Name", "AQS_PARAMETER_DESC", "COUNTY", "SITE_LATITUDE", "SITE_LONGITUDE")]
head(EPAair_O3_NC2018_processed)

EPAair_O3_NC2019_processed <- EPAair_O3_NC2019_raw[,c("Date", "DAILY_AQI_VALUE",
"Site.Name", "AQS_PARAMETER_DESC", "COUNTY", "SITE_LATITUDE", "SITE_LONGITUDE")]
head(EPAair_O3_NC2019_processed)
...
```{r}
write.csv(EPAair_O3_NC2018_processed, row.names = FALSE, file = 
            "./Data/Processed/EPAair_O3_NC2018_processed.csv")
write.csv(EPAair_O3_NC2019_processed, row.names = FALSE, file = 
...
EPAAir_AQ_MasterDataSet <- rbind(EPAair_O3_NC2018_processed, EPAair_O3_NC2019_processed, 
...
```{r}
#Filtering combined dataset to only include the rows with the given sites. 
#Saving in new object.
EPAAir_AQ_MasterDataSet_wrangled <- EPAAir_AQ_MasterDataSet %>%
...
count(EPAAir_AQ_MasterDataSet_wrangled, EPAAir_AQ_MasterDataSet_wrangled$Site.Name)
glimpse(EPAAir_AQ_MasterDataSet_wrangled)

#Applying the split-apply-combine strategy
EPAAir_AQ_MasterDataSet_grouped <- 
  EPAAir_AQ_MasterDataSet_wrangled %>%
...
> Answer: 
The dimensions with na.omit are [1270,5] whereas with drop_na(), it's [1431,5]. This is because na.omit removes all NAs from the dataframe whereas drop_na only drops the NAs in the specified column.
#CHECK!!!