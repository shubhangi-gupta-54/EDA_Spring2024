...
summary(NTL.phys.data$lakename) #how many instances of each factor that we have
NTL.phys.data.PeterPaul <- 
  filter(NTL.phys.data, lakename %in% c("Paul Lake", "Peter Lake")) # %in% is for subsetting
...
NTL.phys.data_Tuesday1990to1999 <- filter(NTL.phys.data, lakename == "Tuesday Lake", year4 %in% c(1990:1999))
head(NTL.phys.data_Tuesday1990to1999)
tail(NTL.phys.data_Tuesday1990to1999)
nrow(NTL.phys.data_Tuesday1990to1999)
```
...
NTL.phys.data.processed <- 
  ...
  select(lakename, sampledate:temperature_C) %>% #this selects the entire column/ variable from the dataset
  mutate(temperature_F = (temperature_C*9/5) + 32)

#Exercise 3: Using a pipe filter NTL.phys.data for Tuesday Lake from 1990 
# through 1999 only for July.
NTL.phys.data_Tuesday1990to1999_July <- 
  NTL.phys.data %>%
  filter(lakename == "Tuesday Lake" & year4 %in% c(1990:1999) & month(sampledate)==7) #can use == only for 1 condition. for a range, use %in% c()
nrow(NTL.phys.data_Tuesday1990to1999_July)
...
NTL.phys.data_MST <- 
  ...
  summarize(Mean_Surface_Temperature = mean(temperature_C))
NTL.phys.data_MST
```
...
#Watch out for any NAs that you may have created as a result of this!
```
