...
PeterPaul.chem.nutrients$month_factor < factor(PeterPaul.chem.nutrients$month, levels = 1:12, labels = month.abb) 
#levels are the total number of things in the factor object - whether they have data in them or not.
#month.abb lists all the 12 months as characters, and the label argument labels the factors.

#Try again: Plot how many Lake observations occur each month
ggplot(PeterPaul.chem.nutrients, aes(x=month_factor))+
  geom_bar()+
  scale_x_discrete(drop=FALSE) #this puts all the factor levels on the x axis, whether they have data or not.
