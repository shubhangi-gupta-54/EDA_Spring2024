...
ggplot(EPAair)+
  geom_boxplot(aes(x=Ozone))+
  labs(x="Ozone", y="Density", fill="Year") #fill puts colour in the legend boxes
  
  stat_summary(geom="text", fun = max, vjust=-1, size=4, label = c("b", "a"))

ggplot(EPAair)+
  geom_violin(aes(x=Ozone), draw_quantiles = 0.5)+
  labs(x="Ozone", y="Density", fill="Year") #fill puts colour in the legend boxes
...
glimpse(EPAair)
PM2.5byOzone_lm <- lm(data=EPAair, PM2.5~Ozone)
summary(PM2.5byOzone_lm)
